...
import { Calendar, Plus, Pencil, Trash2, AlertCircle, Loader2, RefreshCw } from 'lucide-react';
...
function HolidayFormModal({ outlets, initial, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.holiday_date) { setError('Tanggal libur wajib diisi'); return; }
    setSaving(true);
    setError(null);
    try {
      let result;
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Contoh: Renovasi, Libur Lokal, Event Internal"
            />
          </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
        ...
        <p className="text-sm text-gray-600 mb-1">
          Hari libur berikut akan dinonaktifkan:
        </p>
        <div className="bg-gray-50 rounded-lg p-3 mb-4 text-sm">
          <p className="font-medium text-gray-800">{holiday.outlet?.name}</p>
          <p className="text-gray-600">{formatDateID(holiday.holiday_date)}</p>
          {holiday.holiday_name && (
            <p className="text-gray-500">{holiday.holiday_name}</p>
          )}
        </div>
        <p className="text-xs text-gray-400 mb-5">
          Data tidak akan dihapus permanen. Hari ini tidak lagi dianggap libur saat order dibuat.
        </p>
    ...
    <div className="page-shell max-w-3xl">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            Atur hari libur operasional per cabang untuk perhitungan order roti yang akurat
          </p>
        </div>
        <button
      ...
      <div className="card p-4 mb-4 flex flex-wrap items-center gap-3">
        ...
        <button
          ...
          className="btn-secondary flex items-center gap-1.5 text-sm pt-5"
          title="Refresh"
        >
        ...
        {!loading && !error && holidays.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 gap-3">
            ...
            <p className="text-sm text-gray-500">
              {outletName
                ? `Belum ada hari libur untuk cabang ${outletName}.`
